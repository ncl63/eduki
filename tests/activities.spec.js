...
test('désignation : difficulté, contenus actifs et réponse', async ({ page }) => {
  ...
  const choiceCount = page.getByRole('slider', { name: /Nombre de choix/ })
  await expect(choiceCount).toHaveValue('2')
  await choiceCount.focus()
  await choiceCount.press('ArrowRight')
  await expect(choiceCount).toHaveValue('3')
  await expect(page.getByText('Nombre de choix (3)', { exact: true })).toBeVisible()
