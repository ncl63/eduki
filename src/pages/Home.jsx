...
const categories = ['Tout voir', 'Lettres', 'Mots', 'Nombres']
  ...
  return <div className="home-shell">
    ...
    <main className="home-main">
      <section id="exercices" className="exercise-section" tabIndex={-1} aria-labelledby="exercises-title">
        ...
        <div className="filters" role="group" aria-label="Filtrer les exercices">{categories.map(item => <button key={item} type="button" aria-pressed={category === item} onClick={() => setCategory(item)}>{item}{item === 'Tout voir' && <span>4</span>}</button>)}</div>
        <p className="sr-only" role="status">{exercises.length} exercice{exercises.length > 1 ? 's' : ''} affiché{exercises.length > 1 ? 's' : ''}</p>
        <div className="exercise-grid">{exercises.map(exercise => <CarteExercice key={exercise.id} exercise={exercise} />)}</div>
      </section>
    ...
    <footer className="home-footer"><span className="footer-brand">eduki<span>.</span></span><p>Apprendre, un exercice à la fois.</p><span>Lettres, mots & nombres</span></footer>
